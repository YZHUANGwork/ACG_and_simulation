...
import smoke_diffusion_analytical as SMOKE
...
hex_colors = cgc.color_row_gradient(BLOCK_part0, cgc.hex_to_rgb("#e8dcb1" ), cgc.hex_to_rgb("#842648"), 
                                    hex_rc_arr, hex_colors, sort = 'col', sigma_color = sigma_color, end_weight= 0.01)  
foreground_points.extend(FRONTWALL)
foreground_points.extend(BLOCK_part0)
...
foreground_points.extend(right_ear+left_ear+eyes)
...
foreground_points.extend(torso)
...
foreground_points.extend(left_foot+right_foot+front_feet)
...
#------read solution-----------------------
RESULT = SMOKE.C_projected_grid()
x, y, t_eval, proj_t = RESULT
X, Y = np.meshgrid(x, y, indexing='ij')   
    
# ── canvas sizing must match IMG_W:IMG_H or a circle stretches into an
DOMAIN_W = x[-1] - x[0]


CANVAS_PHYSICAL_H_list = [DOMAIN_W*0.6,DOMAIN_W*0.4, DOMAIN_W]
def obtain_canvas_projection(CANVAS_PHYSICAL_H):
    CANVAS_PHYSICAL_W = CANVAS_PHYSICAL_H * (IMG_W / IMG_H)
    canvas_physical_x_range = (0, CANVAS_PHYSICAL_W)
    canvas_physical_y_range = (0, CANVAS_PHYSICAL_H)
    return CANVAS_PHYSICAL_W, canvas_physical_x_range, canvas_physical_y_range

CANVAS_PHYSICAL_W_list = []
canvas_physical_x_range_list = []
canvas_physical_y_range_list = []
for canvas_h in CANVAS_PHYSICAL_H_list:
    w, x_range, y_range = obtain_canvas_projection(canvas_h)
    CANVAS_PHYSICAL_W_list.append(w)
    canvas_physical_x_range_list.append(x_range)
    canvas_physical_y_range_list.append(y_range)
 
OFFSET_X_list = [
    CANVAS_PHYSICAL_W_list[0] / 2 - CANVAS_PHYSICAL_W_list[0] * 0.1,
    CANVAS_PHYSICAL_W_list[1] / 2 + CANVAS_PHYSICAL_W_list[1] * 0.3,
    CANVAS_PHYSICAL_W_list[2] / 2 + CANVAS_PHYSICAL_W_list[2] * 0.4,
]
OFFSET_Y_list = [
    CANVAS_PHYSICAL_H_list[0] / 2 + CANVAS_PHYSICAL_H_list[0] * 0.3,
    CANVAS_PHYSICAL_H_list[1] / 2,
    CANVAS_PHYSICAL_H_list[2] / 2 + CANVAS_PHYSICAL_H_list[2] * 0.4,
]
 
# ── PROJECT ONTO THE HEX GRID: physical grid -> pixel -> hex index, once
# per canvas placement -- three separate hex_pos_to_grid_idx maps, applied
# one by one in update() below
rc_to_idx = {rc: i for i, rc in enumerate(hex_rc_arr)}
hex_pos_to_grid_idx_list = []
for i in range(len(CANVAS_PHYSICAL_H_list)):
    grid_hex_rc = cg.world_metres_to_hex_index(
        X.ravel() + OFFSET_X_list[i], Y.ravel() + OFFSET_Y_list[i], detail_info,
        canvas_physical_x_range=canvas_physical_x_range_list[i],
        canvas_physical_y_range=canvas_physical_y_range_list[i],
    )
    hex_to_grid_idx = {}
    for flat_i, rc in enumerate(grid_hex_rc):
        hex_to_grid_idx.setdefault(rc, []).append(flat_i)
    hex_pos_to_grid_idx_list.append({
        rc_to_idx[rc]: idxs
...
CUTOFF = 0.1
global_peak = proj_t[..., 0].max()
 
 
# ── UPDATE ──────────────────────────────────────────────────────────────
Phase0_end = 10
frames_per_phase = len(t_eval)
PHASE_START_FRAMES = [0,int(frames_per_phase*0.8), int(frames_per_phase*0.8)*2] 
total_frames = len(t_eval)*len(CANVAS_PHYSICAL_H_list)
def update(snapshot):
    if snapshot <Phase0_end:
        ...
        current_hex_colors = base_hex_colors.copy()
        phase = max(i for i, start in enumerate(PHASE_START_FRAMES) if current_snapshot >= start)
        local_snapshot = min(current_snapshot - PHASE_START_FRAMES[phase], frames_per_phase - 1) 
        hex_pos_to_grid_idx = hex_pos_to_grid_idx_list[phase]
        proj_flat = proj_t[..., local_snapshot].ravel()
        positions = np.array(list(hex_pos_to_grid_idx.keys()))
        ratios = np.array([min(proj_flat[hex_pos_to_grid_idx[pos]].mean() / global_peak * 1.4, 1.)
                            for pos in positions])

        select_pos = np.zeros(n_hex, dtype=bool)
        select_pos[positions[ratios >= CUTOFF]] = True

        if select_pos.any():
            alpha = np.zeros(n_hex)
            alpha[positions] = ratios
            # each active hex gets its own slightly-varied smoke shade, same call
            # pattern as select_CHAR's cgc.select_normal_color(mask, base, sigma)
            current_hex_colors[select_pos] = (
                (1 - alpha[select_pos])[:, None] * current_hex_colors[select_pos]
                + alpha[select_pos][:, None] * cgc.select_normal_color(select_pos, cgc.hex_to_rgb("#6305b6"), np.ones(3)*sigma_color)
            )
...
OUTPUT_FILE = os.path.join(OUTPUT_FOLDER, 'omen_animation.mp4')
